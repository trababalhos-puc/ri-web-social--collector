...
if __name__ == '__main__':
    try:
        ...
        logger.info(f"Mapeamento de arquivos HTML concluído. Arquivo de saída: {mapper.output_file}")

        analyzer = Index(mapper.output_file).start()

        if analyzer:
            analyzer.compare_indices()
            logger.info("Processamento completo e análise de índices finalizada com sucesso.")
        else:
